...
public sealed partial class AdminMessaging
{
    ...
    private void CommandSomething(CCSPlayerController? client, CommandInfo info)
    {
        if (info.ArgCount <= 2)
        {
            if (client == null)
            {
                Server.PrintToConsole(LocalizeString("", info.ArgByIndex(0).ToLower().Replace("css_", "")));
            }
            else
            {
                client.PrintToChat(LocalizeWithPluginPrefixForPlayer(client, "", info.ArgByIndex(0).ToLower().Replace("css_", "")));
            }
            return;
        }
        ...
        if (hasTypedTargets && targets.Count() >= 2)
        {
            foreach(CCSPlayerController target in targets) {
                ...
                target.PrintToChat(LocalizeStringForPlayer(target, "", executorName, message.ReplaceColorTags()));
            }
        }
        else
        {
