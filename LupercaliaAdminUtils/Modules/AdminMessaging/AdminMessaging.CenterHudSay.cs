...
public sealed partial class AdminMessaging
{
    ...
    private void CommandCenterHudSay(CCSPlayerController? client, CommandInfo info)
    {
        if (info.ArgCount <= 2)
        {
            if (client == null)
        ...
        if (hasTypedTargets && targets.Count() >= 2)
        {
            foreach(CCSPlayerController target in targets) {
                ...
                target.PrintToCenter(LocalizeStringForPlayer(target, "AdminMessaging.CenterHudSay.Message", executorName, message.ReplaceColorTags()));
            }
        }
        else
        {
